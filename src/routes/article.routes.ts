    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (!title || !content || !locationId)
                return res
                    .status(400)
                    .json({ success: false, message: "title, content, locationId wajib diisi" });
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (existingPost.userId !== authenticatedUserId)
                return res
                    .status(403)
                    .json({ success: false, message: "Forbidden: bukan pemilik artikel" });
    ...
    try {
        ...
        return res.json({
            ...
            comments: comments.map((c) => ({
                id: c.id,
                userId: c.userId,
                username: c.username || c.User?.username || "匿名",
                content: c.content,
...
router.post(
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(500).json({ success: false, message: "Gagal membuat komentar" });
        }
    }
);
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (existing.userId !== userId)
                return res
                    .status(403)
                    .json({ success: false, message: "Forbidden: bukan pemilik komentar" });
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (existing.postId !== postId)
                return res
                    .status(400)
                    .json({ success: false, message: "Post ID tidak cocok dengan komentar" });
            if (existing.userId !== userId)
                return res
                    .status(403)
                    .json({ success: false, message: "Forbidden: bukan pemilik komentar" });
